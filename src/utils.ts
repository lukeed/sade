...
const NL = '\n'
function format(arr:string[][]):string|string[] {
	if (!arr.length) return ''
	let len = maxLen(arr.map(x=>x[0])) + GAP
	let join = (a:string[])=>a[0] + ' '.repeat(len - a[0].length) + a[1] + (a[2] == null ? '' : `  (default ${a[2]})`)
	return arr.map(join)
}
function maxLen(arr:string[]) {
...
function section(str:string, arr:string|string[], fn:(str:string)=>string) {
...
export function help(
	bin:string,
	tree:Record<string, { options:string[][], usage:string, describe:string[], alibi:string, examples:string[] }>,
...
export const error = function (bin:string, str:string, num = 1) {
...
export const sentences = function (str:string) {
